...
<li class="flex-grow-1 fullthonghtin sb-mobile sb-{{ Str::slug($title) }} {{ $wrapperClass ?? '' }}"
    @style(["--fb-background-color: $bgColor" => $bgColor, "margin: {$marginBetween}px 0" => $marginBetween])
>
    <div class="sb-mask is-style3-info">
        <a href="{{ $url }}" @if($floatingButton->get('open_in_the_new_tab')) target="_blank" @endif
            class="ring-animation is-style3-info">
            ...
            {{-- Luôn hiển thị label như khi hover --}}
            <div class="sb-label force-always sb-show">{{ $title }}</div>
        </a>
    </div>
</li>

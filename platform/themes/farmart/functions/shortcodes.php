...
app('events')->listen(RouteMatched::class, function (): void {
    ...
    //  -------------- Shortcode Customer By TIVATECH  ------------------

    add_shortcode('introduce-box', __('Giới Thiệu'), __('Thông tin giới thiệu Cty'), function (Shortcode $shortcode) {
        return Theme::partial('shortcodes.introduce.index', compact('shortcode'));
    });

    ShortcodeFacade::setAdminConfig('introduce-box', function (array $attributes) {
        return ShortcodeForm::createFromArray($attributes)
            ->withLazyLoading()
            ->add('style', 'html', [
                'label' => __('Style'),
                'html' => view('vendor.core.shortcode.fields.style-selector', [
                    'name' => 'style',
                    'value' => $attributes['style'] ?? 'style1',
                    'label' => __('Style'),
                    'theme_path' => '/themes/farmart/images/shortcodes/introduce',
                  'total' => 2,    // có 4 hình thì đặt 4
            ...
            ->add('button_link', 'text', [
                'label' => __('Đường dẫn trang'),
            ])
    ...
    add_shortcode('actual-pictures', __('Hình Ảnh Thực Tế'), __('Hình Ảnh Thực Tế'), function (Shortcode $shortcode) {
        return Theme::partial('shortcodes.actual-pictures.index', compact('shortcode'));
    });
    
    ShortcodeFacade::setAdminConfig('actual-pictures', function (array $attributes) {
        $numberTabs = (int)($attributes['number_tabs'] ?? 3);
    
        //  Gộp attributes và dữ liệu từ request khi form submit lại
        $attributes = array_merge($attributes, request()->all());
    
        return ShortcodeForm::createFromArray($attributes)
            ->withLazyLoading()
            ->add('title', 'text', ['label' => 'Tiêu đề'])
            ->add('number_tabs', 'select', [
                'label' => 'Số lượng tab',
                'choices' => array_combine(range(1, 20), range(1, 20)),
                'attributes' => ['onchange' => 'this.form.submit()']
            ])
            ->add('tabs', 'html', [
                'label' => __('Danh sách Tabs'),
                'html' => view('vendor.core.shortcode.fields.tab-accordion', [
                    'attributes' => $attributes,
                    'name_prefix' => 'tab',
                    'number' => $numberTabs,
                    'fields' => ['title', 'img'],   //  tùy biến
                ])->render()
            ])
            ->add('background_color', 'color', [
                'label' => __('Background Color'),
    ...
    add_shortcode('partner', __('Đối tác'), __('Đối tác Khách hàng'), function (Shortcode $shortcode) {
        return Theme::partial('shortcodes.partner.index', compact('shortcode'));
    });
    
    ShortcodeFacade::setAdminConfig('partner', function (array $attributes) {
        $numberTabs = (int) ($attributes['number_tabs'] ?? 3);
    
        //  Gộp lại nếu người dùng submit lại form
        $attributes = array_merge($attributes, request()->all());
        ...
        return ShortcodeForm::createFromArray($attributes)
            ...
            ->add('suptitle', 'text', [
                'label' => __('Phụ đề'),
                'value' => Arr::get($attributes, 'suptitle'),
            ])
            ->add('number_tabs', 'select', [
                'label' => __('Số lượng hình ảnh'),
                'choices' => array_combine(range(1, 20), range(1, 20)),
                'selected' => $numberTabs,
                'attributes' => ['onchange' => 'this.form.submit()'],
            ])
            ->add('tabs', 'html', [
                'label' => __('Danh sách đối tác'),
                'html' => view('vendor.core.shortcode.fields.tab-accordion', [
                    'attributes' => $attributes,
                    'name_prefix' => 'tab',
                    'number' => $numberTabs,
            ...
            ->add('autoplay_speed', 'customSelect', [
                'label' => __('Tốc độ chạy (ms)'),
                'choices' => theme_get_autoplay_speed_options(),   //  ví dụ [1000, 2000, 3000, 5000]
                'selected' => Arr::get($attributes, 'autoplay_speed', 3000),
            ])
            ->add('background_color', 'color', [
                'label' => __('Màu nền'),
    ...
    add_shortcode('subitem', __('Subitem '), __('Nhiều sì tai dùng tới công chuyện'), function (Shortcode $shortcode) {
        return Theme::partial('shortcodes.subitem.index', compact('shortcode'));
    });
    
    ShortcodeFacade::setAdminConfig('subitem', function (array $attributes) {
        $numberTabs = (int)($attributes['number_tabs'] ?? 3);
        ...
        return ShortcodeForm::createFromArray($attributes)
            ...
            ->add('style', 'html', [
                ...
                'html' => view('vendor.core.shortcode.fields.style-selector', [
                    ...
                    'value' => $attributes['style'] ?? 'style1',
                    'label' => __('Style'),
                    'theme_path' => '/themes/farmart/images/shortcodes/subitem',
                  'total' => 3,    // có 4 hình thì đặt 4
                    'attributes' => $attributes
                ])->render()
            ])
            ->add('title', 'text', ['label' => 'Tiêu đề'])
            ...
            ->add('tabs', 'html', [
                ...
                'html' => view('vendor.core.shortcode.fields.tab-accordion', [
                    ...
                    'name_prefix' => 'tab',
                    'number' => $numberTabs,
                    'fields' => ['title','description','url', 'img'],   //  tùy biến
                ])->render()
            ])
            ->add('background_color', 'color', [
                'label' => __('Background Color'),
